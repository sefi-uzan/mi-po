      ...
      <div className="flex-1">
       ...
       <h1 className="text-2xl font-semibold text-zinc-100">{building.name}</h1>
        <p className="text-sm text-zinc-400 mb-3">
          {t('BuildingHeader.created')} {new Date(building.createdAt).toLocaleDateString('en-US', {
            year: 'numeric',
            month: 'long',
            day: 'numeric'
          })}
        ...
        <div className="flex items-center gap-2">
          {currentUserId && onTogglePresence && (
            ...
            >
              ...
              <span className="text-xs hidden sm:inline">
                {isCurrentUserPresent ? t('BuildingHeader.markAway') : t('BuildingHeader.markPresent')}
              </span>
            </button>
          )}
          <button
          ...
          >
            ...
            <span className="text-xs hidden sm:inline">{t('BuildingHeader.invite')}</span>
          </button>
        </div>
       </div>
        ...
        {totalResidents > 0 && (
          <div className="flex items-center gap-3">
            <div className="flex flex-col items-center bg-blue-500/20 border border-blue-500/30 rounded-lg px-3 py-2 w-[70px] h-[50px] justify-center">
              ...
              <span className="text-blue-300 text-xs font-medium">{t('BuildingHeader.total')}</span>
            </div>
            <div className="flex flex-col items-center bg-green-500/20 border border-green-500/30 rounded-lg px-3 py-2 w-[70px] h-[50px] justify-center">
              ...
              <span className="text-green-300 text-xs font-medium">{t('BuildingHeader.present')}</span>
            </div>
            <div className="flex flex-col items-center bg-red-500/20 border border-red-500/30 rounded-lg px-3 py-2 w-[70px] h-[50px] justify-center">
              ...
              <span className="text-red-300 text-xs font-medium">{t('BuildingHeader.away')}</span>
            </div>
          </div>
        )}
      </div>
