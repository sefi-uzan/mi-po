...
export const authRouter = j.router({
  ...
  createOwner: publicProcedure
    .input(
      z.object({
        ...
        buildingName: z.string().min(1).max(20),
      })
    )
    .post(async ({ c, ctx, input }) => {
      ...
      if (!presence[0]) {
        throw new HTTPException(500, {
          message: "Failed to create presence status",
        });
      }
      ...
      const token = await sign(
        {
          ...
          role: "owner",
          exp: Math.floor(
            (Date.now() + parseInt(JWT_EXPIRATION_TIME, 10)) / 1000
          ),
  ...
  loginPhone: publicProcedure
    .input(
      z.object({ phoneNumber: z.string().min(1), code: z.string().length(6) })
    )
    .post(async ({ c, ctx, input }) => {
      const {
        TWILIO_VERIFY_SERVICE_SID,
  ...
  createResident: publicProcedure
    .input(
      z.object({
        ...
        name: z.string().min(1).max(20),
        details: z.string().optional(),
        buildingInviteCode: z.string().min(1),
      })
    )
    .post(async ({ c, ctx, input }) => {
      const { db, twilio } = ctx;
      ...
      if (resident[0]) {
        throw new HTTPException(409, {
          message: "A user with this name already exists in this building",
        });
      }
    ...
    .post(async ({ c, ctx, input }) => {
      ...
      if (!resident[0]) {
        throw new HTTPException(404, {
          message: "User not found in this building",
        });
      }

      if (resident[0].phoneVerified) {
        throw new HTTPException(400, {
          message: "Please log in with your phone number",
        });
      }
    ...
    .post(async ({ c, ctx, input }) => {
      ...
      try {
        ...
        if ((error as { code?: number }).code === 60200) {
          ...
          throw new HTTPException(400, {
            message: "SMS delivery failed. Please check your phone number.",
          });
        } else if ((error as { code?: number }).code === 60410) {
          throw new HTTPException(400, {
            message:
              "Phone number temporarily blocked. Please try a different number.",
          });
        }
  ...
  logout: publicProcedure.post(async ({ c }) => {
